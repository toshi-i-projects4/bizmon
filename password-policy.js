...
export function validateBizmonPassword(password) {
  if (!password) return "パスワードを入力してください。";
  if (password.length < PASSWORD_MIN_LENGTH) return `パスワードは${PASSWORD_MIN_LENGTH}文字以上で設定してください。`;
  if (password.length > PASSWORD_MAX_LENGTH) return `パスワードは${PASSWORD_MAX_LENGTH}文字以内で設定してください。`;
  return "";
}
