window.resultStep1V3Executed = true;

var saveStatus = document.getElementById("saveStatus");
if (saveStatus) {
  saveStatus.textContent = "STEP2_RETRY起動：外部JavaScriptは実行されました。結果情報を表示します。";
}
...
try {
  ...
  if (questionEl) questionEl.innerText = questionText ? "問題：" + questionText : "問題情報がありません。";
  if (resultEl) resultEl.innerText = resultText ? "判定：" + resultText : "判定結果がありません。";
  if (detailEl) {
    detailEl.innerText = (selectedChoice && correctChoice)
      ? "あなたの回答：" + selectedChoice + " / 正解：" + correctChoice
      : "回答情報がありません。";
  }
  if (explanationEl) explanationEl.innerText = explanationText ? "解説：" + explanationText : "解説はありません。";

  if (saveStatus) {
    saveStatus.textContent =
      "STEP2_RETRY_OK：外部JavaScriptで結果表示に成功しました。
" +
...
" +
  ...
  if (saveStatus) {
    saveStatus.textContent =
      "STEP2_RETRY_ERROR：結果表示処理中にエラーが発生しました。
" +
